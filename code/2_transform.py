...
survey_data['_country'] = survey_data['What county do you work in?'].apply(pl.clean_country_usa)

survey_states_combined = survey_data.merge(states_data, left_on="If you are in the U.S., what state do you work in?", right_on='State', how='inner')

survey_states_combined['_full_city'] = survey_states_combined['What city do you work in?'] +', '+ survey_states_combined['Abberviation'] +', ' + survey_states_combined['_country']

combined = survey_states_combined.merge(col_data, left_on['year', '_full_city'], right_on=['year', 'City'], how='inner')

combined['_annual_salary_cleaned'] = combined['What is your annual salary? (You will indicate the currency in a later question. If you are part-time or hourly, please enter an annualized equivalent -- what you would earn if you worked the job 40 hours a week for a year.)'].apply(pl.clean_currency)
